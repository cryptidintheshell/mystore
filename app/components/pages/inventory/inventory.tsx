...
export default function Inventory() {
	...
	return (
		<main className="bg-slate-50 h-full w-full px-5 p-3 overflow-y-auto">
			<div className="flex flex-col gap-3">
				...
				{/* table */}
				<div className={`flex p-5 gap-5 w-full min-h-150 h-auto text-center rounded-2xl shadow-sm border border-slate-200 
				${items.length === 0 ? "justify-center items-center" : ""}`}>
					...
					{ items.length === 0  ? 
						...
						items.map((item)=> (
						<ItemCard key={item.barcode} name={item.name} price={item.price.toFixed(2)} amount={item.amount} />
					))}
				</div>
